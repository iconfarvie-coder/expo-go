...
import { _isLocalHostname } from './CorsMiddleware';
...
export interface RemotePeerWarning {
  ...
  onRemotePeer: (remoteAddress: string | undefined, userAgent: string | undefined) => void;
}
...
export function createRemotePeerWarning(): RemotePeerWarning {
  ...
  return {
    middleware: (req, _res, next) => {
      handlePeer(req.socket?.remoteAddress, asString(req.headers['user-agent']));
      next();
    },
